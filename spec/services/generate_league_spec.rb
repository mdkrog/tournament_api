...
RSpec.describe GenerateLeague, type: :service do
  describe "given a tournament it must create an associated league" do
    it "creates a league with only 1 group" do
      ...
      league_generator = GenerateLeague.new(tournament: tournament, number_of_groups: 1, group_assignment_strategy: group_assignment_strategy)
      league = league_generator.generate

      expect(league.tournament).to eq(tournament)
      expect(league.groups.count).to eq(1)
      expect(group_assignment_strategy).to have_received(:assign).with([1], [1,2,3,4]).ordered
    ...
    it "creates a league with 3 group" do
      ...
      league_generator = GenerateLeague.new(tournament: tournament, number_of_groups: 3, group_assignment_strategy: group_assignment_strategy)
      league = league_generator.generate

      expect(league.groups.count).to eq(3)
      expect(group_assignment_strategy).to have_received(:assign).with([1,2,3], [6,5,4,3,2,1]).ordered
      mocked_lps.each do |lp|
        expect(lp).to have_received(:save)
      end
    end
